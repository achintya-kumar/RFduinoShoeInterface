...
void setup()
{
  ...
  /* Initializing the ToF-distance-sensor */
  tof.init();
  ...
  Serial.print("activating BLE");
  RFduinoBLE.deviceName = "ShoeInterface";
...
void loop()
{
  ...
  if (!AHRS) {
    ...
    if (delt_t > 50) { // update LCD once per half-second independent of read rate
      ...
      pitch *= 180.0f / PI;
      yaw   *= 180.0f / PI;
      yaw   -= 3.25; /* Declination at Potheri, Chennail ,India  Model Used:  IGRF12  Help
      ...
      yaw += 180;
      byte packet[] = {highByte((int)ax), lowByte((int)ax), highByte((int)ay), lowByte((int)ay), highByte((int)az), lowByte((int)az), highByte((int)mx), lowByte((int)mx),
                       highByte((int)my), lowByte((int)my), highByte((int)mz), lowByte((int)mz), highByte((int)distance), lowByte((int)distance), highByte((int)yaw), lowByte((int)yaw), highByte((int)pitch), lowByte((int)pitch), highByte((int)roll), lowByte((int)roll)
                      };
      RFduinoBLE.send((char*)packet, sizeof(packet) + 1);
